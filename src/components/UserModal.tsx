...
export const UserModal: React.FC<UserModalProps> = ({
    ...
    try {
      ...
      if (res?.user) {
        ...
        onUpdateProfile({
          ...
          photoURL: newPhoto,
          googleUid: u.uid,
          role: isSysAdmin ? 'admin' : (TIER_TO_ROLE[selectedTierId] || 'sr_manager'),
          tierId: isSysAdmin ? 'tier_0' : selectedTierId,
  ...
  const handleSaveProfile = (e: React.FormEvent) => {
    ...
    if (cleanEmail === SYSTEM_ADMIN_EMAIL.toLowerCase() && selectedTierId !== 'tier_0' && !isSystemAdmin(profile)) {
      setShowAdminPassModal(true);
      return;
    }

    const isRootAdmin = selectedTierId === 'tier_0' || isSystemAdmin(profile);
            ...
            <div className="mt-3 p-3.5 rounded-2xl bg-[#0f282f] text-white border border-teal-500/30 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 animate-in slide-in-from-top-2 duration-200">
              <div className="flex-1 space-y-0.5">
                ...
                <p className="text-[11px] text-teal-100/80">
                  Enter authorization passcode to elevate active session:
                </p>
                {adminPassError && (
              ...
              <div className="space-y-3">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={e => {
                      if (e.target.value === 'tier_0' && !isSystemAdmin(profile)) {
                        setShowAdminPassModal(true);
                        return;
                      }
                  ...
                  >
                    {roleTiers
                      .filter(tier => !tier.isHidden || selectedTierId === tier.id || isSystemAdmin(profile))
                      .map(tier => (
