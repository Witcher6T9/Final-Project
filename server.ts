...
async function startServer() {
  ...
  app.get('/api/internal/gemini-keys/status', (req, res) => {
    res.json(geminiKeyManager.getStatus());
  });

  app.post('/api/internal/gemini-keys/auto-update', async (req, res) => {
    try {
      const { key, keys, action = 'update' } = req.body || {};
  ...
  app.post('/api/internal/gemini-keys/reload', (req, res) => {
    try {
  ...
  app.post('/api/chat', async (req, res) => {
    try {
      ...
      if (Array.isArray(messages) && messages.length > 0 && geminiKeyManager.getStatus().hasActiveKey) {
        try {
...
Return ONLY valid JSON matching this exact structure:
{
  ...
  app.post('/ai-team-assignment', async (req, res) => {
    try {
      ...
      if (geminiKeyManager.getStatus().hasActiveKey) {
        try {
